...
export async function GET(request: NextRequest) {
  ...
  if (action === 'download' && fileIdParam) {
    ...
    try {
      ...
      // Forward Range từ browser -> Google Drive. Đây là điểm quan trọng để
      // HTML5 video có thể seek/buffer mượt trên cả điện thoại và desktop.
      const requestRange = request.headers.get('range')
      if (requestRange) {
        driveHeaders['Range'] = requestRange
      }
      ...
      const status = requestRange && driveRes.status === 206 ? 206 : 200
